...
-- Tambahkan setelah pembuatan Window:
local Tabs = {
    Main = Window:CreateTab("Main", 1),
    Player = Window:CreateTab("Player", 2),
    Auto = Window:CreateTab("Auto", 3),
    Visuals = Window:CreateTab("Visuals", 4),
    ESP = Window:CreateTab("ESP", 5),
    Utility = Window:CreateTab("Utility", 6),
    Teleport = Window:CreateTab("Teleport", 7),
    Settings = Window:CreateTab("Settings", 8),
}
...
local BoxticketTypeDropdown = Tabs.ESP:Dropdown({
    Title = "Box Type",
    Values = {"2D", "3D"},
    Value = "2D",
    Callback = function(value)
...
-- (Tambahkan kode ini di akhir script, sebelum baris Fluent:Notify terakhir)

-- Buat ScreenGui untuk tombol Show/Hide UI
local showHideGui = Instance.new("ScreenGui")
showHideGui.Name = "ShowHideUI_Gui"
showHideGui.Parent = playerGui -- Gunakan playerGui yang sudah didefinisikan sebelumnya
showHideGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
showHideGui.ResetOnSpawn = false -- Agar tidak hilang saat spawn ulang

-- Buat ImageButton sebagai tombol lingkaran dengan emoji
local showHideButton = Instance.new("ImageButton")
showHideButton.Name = "ShowHideButton"
showHideButton.Size = UDim2.new(0, 30, 0, 30) -- Ukuran persegi untuk membuat lingkaran
showHideButton.Position = UDim2.new(0, 10, 0, 10) -- Posisi awal (kiri atas)
showHideButton.BackgroundColor3 = Color3.fromRGB(50, 50, 50) -- Warna latar belakang tombol
showHideButton.BorderSizePixel = 1
showHideButton.BorderColor3 = Color3.fromRGB(100, 100, 100)
-- Gunakan emoji sebagai teks
showHideButton.Text = "" -- Awalnya kosong, akan diisi oleh fungsi update
showHideButton.TextScaled = true -- Skalakan teks agar pas
showHideButton.Font = Enum.Font.Gotham -- Font yang biasanya mendukung emoji
showHideButton.TextColor3 = Color3.fromRGB(255, 255, 255) -- Warna teks (emoji) putih
showHideButton.TextTransparency = 0.2 -- Agar sedikit transparan
showHideButton.Parent = showHideGui

-- Buat UICorner untuk membuat tombol menjadi lingkaran
local buttonCorner = Instance.new("UICorner")
buttonCorner.CornerRadius = UDim.new(1, 0) -- CornerRadius 100% membuatnya lingkaran
buttonCorner.Parent = showHideButton

-- (Opsional) Buat UIStroke untuk garis pinggir yang lebih tebal
local buttonStroke = Instance.new("UIStroke")
buttonStroke.Color = Color3.fromRGB(100, 100, 100) -- Warna garis pinggir
buttonStroke.Thickness = 2 -- Ketebalan garis
buttonStroke.Parent = showHideButton

-- Fungsi untuk memperbarui tampilan tombol berdasarkan status UI
local function updateButtonDisplay()
    if Window.Instance and Window.Instance.Enabled then
        showHideButton.Text = "🔘" -- Emoji saat UI aktif (Lampu menyala)
        showHideButton.TextTransparency = 0.2 -- Sedikit transparan saat aktif
        showHideButton.BackgroundColor3 = Color3.fromRGB(0, 100, 0) -- Hijau
    else
        showHideButton.Text = "🔳" -- Emoji saat UI tidak aktif (Lampu mati)
        showHideButton.TextTransparency = 0.5 -- Lebih transparan saat disembunyikan
        showHideButton.BackgroundColor3 = Color3.fromRGB(100, 0, 0) -- Merah
    end
end

-- Tambahkan efek hover (opsional)
showHideButton.MouseEnter:Connect(function()
    showHideButton.TextTransparency = 0 -- Tidak transparan saat hover
end)

showHideButton.MouseLeave:Connect(function()
    updateButtonDisplay() -- Kembali ke transparansi default berdasarkan status UI
end)
...
showHideButton.MouseButton1Click:Connect(function()
    if Window.Instance then
        ...
        updateButtonDisplay() -- Perbarui emoji dan warna setelah diklik
    end
end)

-- Fungsi untuk membuat ImageButton bisa di-drag
local function makeDraggable(element)
    element.Active = true -- Aktifkan interaksi mouse
    element.Draggable = true -- Aktifkan fitur draggable bawaan Roblox
end

-- Terapkan fungsi draggable ke tombol
makeDraggable(showHideButton)

-- Panggil fungsi sekali untuk mengatur tampilan awal tombol
updateButtonDisplay()

-- (Lanjutkan dengan kode yang sudah ada di akhir script Anda)
-- Notifikasi
Fluent:Notify({
    Title = "Zen Hub Loaded",
    Content = "UI telah diubah menjadi Fluent. Fitur-fitur utama telah dimasukkan.",
    Duration = 3
})
