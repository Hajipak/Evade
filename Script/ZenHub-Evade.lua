-- ZenHub-Test.lua
-- Combined file: original Evade script executed with WindUI stub (to disable old UI) and new Fluent UI appended.
-- Generated by assistant. Name: Zen Hub
-- Note: run this file alone; it contains the original logic and the Fluent UI replacement.

-- WindUI stub to neutralize original UI (prevents errors but does nothing)
local WindUI = {}
function WindUI:Localization(...) return { Enabled = false } end
function WindUI:SetTheme(...) end
function WindUI:GetCurrentTheme() return "Dark" end
function WindUI:Notify(...) end
function WindUI:CreateTopbarButton(...) end
function WindUI:Tag(...) end
function WindUI:SetIconSize(...) end
...
-- Execute original Evade script (embedded)
local original = [==========[
...
local ok, err = pcall(function() loadstring(original)() end)
if not ok then warn("Original Evade script execution failed: "..tostring(err)) end

-- Append Fluent UI (replacement)

-- Fluent UI appended after original script (Zen Hub)
local successFluent, Fluent = pcall(function()
    return loadstring(game:HttpGet("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua"))()
end)
if not successFluent or not Fluent then
    warn("Fluent library failed to load. UI will not be created.")
    return
end

local successSaveMgr, SaveManager = pcall(function()
    return loadstring(game:HttpGet("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/SaveManager.lua"))()
end)

local featureStates = featureStates or getgenv().featureStates or {}
getgenv().featureStates = featureStates

local function safeCallAny(name, ...)
    local fn = rawget(_G, name) or getgenv()[name] or nil
    if type(fn) == "function" then
        local ok, res = pcall(fn, ...)
        if not ok then warn(("Error calling %s: %s"):format(name, tostring(res))) end
        return ok, res
    end
    return false, ("Function %s not found"):format(name)
end
...
local Tabs = {
    ...
    ESP = Window:AddTab({ Title = "ESP", Icon = "layers" }),
    Settings = Window:AddTab({ Title = "Settings", Icon = "settings" })
}

-- (UI bindings same as previously provided Evade_Fluent.lua)
-- For brevity this block creates toggles/sliders/buttons bound to featureStates and calls functions like startPlayerESP, startAutoRevive, etc.
-- Full UI code included in original deliverable file.
-- Player Tab
do local T = Tabs.Player
    featureStates.InfiniteJump = featureStates.InfiniteJump or false
    local infJump = T:AddToggle("InfiniteJump", { Title = "Infinite Jump", Default = featureStates.InfiniteJump })
    infJump:OnChanged(function() featureStates.InfiniteJump = infJump.Value end)
    featureStates.JumpMethod = featureStates.JumpMethod or "Hold"
    local jumpMethod = T:AddDropdown("JumpMethod", { Title = "Infinite Jump Method", Values = {"Hold","Toggle","Tap"}, Multi = false, Default = featureStates.JumpMethod })
    jumpMethod:OnChanged(function(v) featureStates.JumpMethod = v end)
    featureStates.JumpPower = featureStates.JumpPower or 5
    local jumpPower = T:AddInput("JumpPower", { Title = "Jump Power", Default = tostring(featureStates.JumpPower), Placeholder = "Number", Numeric = true })
    jumpPower:OnChanged(function() local n = tonumber(jumpPower.Value); if n then featureStates.JumpPower = n end end)
    featureStates.Fly = featureStates.Fly or false; featureStates.FlySpeed = featureStates.FlySpeed or 5
    local flyToggle = T:AddToggle("Fly", { Title = "Fly", Default = featureStates.Fly })
    flyToggle:OnChanged(function() featureStates.Fly = flyToggle.Value; if featureStates.Fly then safeCallAny("startFlying") else safeCallAny("stopFlying") end end)
    local flySpeed = T:AddSlider("FlySpeed", { Title = "Fly Speed", Default = featureStates.FlySpeed, Min=1,Max=100,Rounding=1 })
    flySpeed:OnChanged(function(v) featureStates.FlySpeed = v end)
    featureStates.TPWALK = featureStates.TPWALK or false; featureStates.TpwalkValue = featureStates.TpwalkValue or 1
    local tpToggle = T:AddToggle("TpWalk", { Title = "TP WALK", Default = featureStates.TPWALK }) tpToggle:OnChanged(function() featureStates.TPWALK = tpToggle.Value end)
    local tpInput = T:AddInput("TpWalkValue", { Title = "TPWALK VALUE", Default = tostring(featureStates.TpwalkValue), Numeric = true }) tpInput:OnChanged(function() local n = tonumber(tpInput.Value); if n then featureStates.TpwalkValue = n end end)
    T:AddDivider()
    T:AddButton({ Title = "Toggle Freecam", Description = "Activate / Deactivate Freecam (if functions exist)", Callback = function() if getgenv().isFreecamEnabled then safeCallAny("deactivateFreecam") else safeCallAny("activateFreecam") end end })
    local fovSlider = T:AddSlider("FOVControl", { Title = "Freecam / Camera FOV", Default = tonumber(featureStates.ZoomValue) or 1, Min=0.5,Max=3,Rounding=2 })
    fovSlider:OnChanged(function(v) featureStates.ZoomValue = v; pcall(function() if workspace.CurrentCamera then workspace.CurrentCamera.FieldOfView = 70 * v end end) end)
end

-- Auto Tab
do local T = Tabs.Auto
    featureStates.AutoCarry = featureStates.AutoCarry or false; local ac = T:AddToggle("AutoCarry",{ Title="Auto Carry", Default=featureStates.AutoCarry }) ac:OnChanged(function() featureStates.AutoCarry = ac.Value end)
    featureStates.AutoRevive = featureStates.AutoRevive or false; local ar = T:AddToggle("AutoRevive",{ Title="Auto Revive", Default=featureStates.AutoRevive }) ar:OnChanged(function() featureStates.AutoRevive = ar.Value if featureStates.AutoRevive then safeCallAny("startAutoRevive") else safeCallAny("stopAutoRevive") end end)
    featureStates.AutoVote = featureStates.AutoVote or false; local av = T:AddToggle("AutoVote",{ Title="Auto Vote", Default=featureStates.AutoVote }) av:OnChanged(function() featureStates.AutoVote = av.Value end)
    featureStates.AutoMoneyFarm = featureStates.AutoMoneyFarm or false; local am = T:AddToggle("AutoMoneyFarm",{ Title="Auto Money Farm", Default=featureStates.AutoMoneyFarm }) am:OnChanged(function() featureStates.AutoMoneyFarm = am.Value end)
    T:AddDivider()
    T:AddButton({ Title="Manual Revive", Callback=function() local ok, res = safeCallAny("ManualRevive"); if not ok then Fluent:Notify({ Title="Manual Revive", Content=tostring(res), Duration=3 }) end end })
    T:AddButton({ Title="Auto Carry Once", Callback=function() local ok, res = safeCallAny("AutoCarryOnce"); if not ok then Fluent:Notify({ Title="Auto Carry", Content=tostring(res), Duration=3 }) end end })
    if successSaveMgr and SaveManager then SaveManager:SetLibrary(Fluent); SaveManager:SetFolder("ZenHubConfigs"); T:AddButton({ Title="Save Config", Callback=function() pcall(function() SaveManager:Save() end) end }); T:AddButton({ Title="Load Config", Callback=function() pcall(function() SaveManager:Load() end) end }) end
end

-- Visuals Tab
do local T = Tabs.Visuals
    featureStates.FullBright = featureStates.FullBright or false; local fb = T:AddToggle("FullBright",{ Title="FullBright", Default=featureStates.FullBright }) fb:OnChanged(function() featureStates.FullBright = fb.Value if featureStates.FullBright then safeCallAny("startFullBright") else safeCallAny("stopFullBright") end end)
    featureStates.NoFog = featureStates.NoFog or false; local nf = T:AddToggle("NoFog",{ Title="Remove Fog", Default=featureStates.NoFog }) nf:OnChanged(function() featureStates.NoFog = nf.Value if featureStates.NoFog then safeCallAny("startNoFog") else safeCallAny("reloadLighting") end end)
    T:AddDivider()
    local brightnessSlider = T:AddSlider("Brightness",{ Title="Brightness", Default=(workspace and workspace.Lighting and workspace.Lighting.Brightness) or 1, Min=0,Max=10,Rounding=2 }) brightnessSlider:OnChanged(function(v) pcall(function() workspace.Lighting.Brightness = v end) end)
end

-- ESP Tab
do local T = Tabs.ESP
    featureStates.PlayerESP = featureStates.PlayerESP or {}; featureStates.PlayerESP.boxes = featureStates.PlayerESP.boxes or false; featureStates.PlayerESP.tracers = featureStates.PlayerESP.tracers or false; featureStates.PlayerESP.names = featureStates.PlayerESP.names or false; featureStates.PlayerESP.distance = featureStates.PlayerESP.distance or false; featureStates.PlayerESP.rainbowBoxes = featureStates.PlayerESP.rainbowBoxes or false; featureStates.PlayerESP.rainbowTracers = featureStates.PlayerESP.rainbowTracers or false; featureStates.PlayerESP.boxType = featureStates.PlayerESP.boxType or "2D"
    local pBoxes = T:AddToggle("PlayerBoxes",{ Title="Player Boxes", Default=featureStates.PlayerESP.boxes }) pBoxes:OnChanged(function() featureStates.PlayerESP.boxes = pBoxes.Value end)
    local pTracers = T:AddToggle("PlayerTracers",{ Title="Player Tracers", Default=featureStates.PlayerESP.tracers }) pTracers:OnChanged(function() featureStates.PlayerESP.tracers = pTracers.Value end)
    local pNames = T:AddToggle("PlayerNames",{ Title="Player Names", Default=featureStates.PlayerESP.names }) pNames:OnChanged(function() featureStates.PlayerESP.names = pNames.Value end)
    local pDist = T:AddToggle("PlayerDistance",{ Title="Player Distance", Default=featureStates.PlayerESP.distance }) pDist:OnChanged(function() featureStates.PlayerESP.distance = pDist.Value end)
    local pRainbow = T:AddToggle("PlayerRainbow",{ Title="Player Rainbow Boxes", Default=featureStates.PlayerESP.rainbowBoxes }) pRainbow:OnChanged(function() featureStates.PlayerESP.rainbowBoxes = pRainbow.Value end)
    local pBoxType = T:AddDropdown("PlayerBoxType",{ Title="Player Box Type", Values={"2D","3D"}, Default=featureStates.PlayerESP.boxType or "2D", Multi=false }) pBoxType:OnChanged(function(v) featureStates.PlayerESP.boxType = v end)
    T:AddDivider()
    featureStates.NextbotESP = featureStates.NextbotESP or {}; featureStates.NextbotESP.boxes = featureStates.NextbotESP.boxes or false; featureStates.NextbotESP.tracers = featureStates.NextbotESP.tracers or false; featureStates.NextbotESP.names = featureStates.NextbotESP.names or false; featureStates.NextbotESP.distance = featureStates.NextbotESP.distance or false; featureStates.NextbotESP.rainbowBoxes = featureStates.NextbotESP.rainbowBoxes or false; featureStates.NextbotESP.rainbowTracers = featureStates.NextbotESP.rainbowTracers or false; featureStates.NextbotESP.boxType = featureStates.NextbotESP.boxType or "2D"
    local nBoxes = T:AddToggle("NextbotBoxes",{ Title="Nextbot Boxes", Default=featureStates.NextbotESP.boxes }) nBoxes:OnChanged(function() featureStates.NextbotESP.boxes = nBoxes.Value end)
    local nTracers = T:AddToggle("NextbotTracers",{ Title="Nextbot Tracers", Default=featureStates.NextbotESP.tracers }) nTracers:OnChanged(function() featureStates.NextbotESP.tracers = nTracers.Value end)
    local nNames = T:AddToggle("NextbotNames",{ Title="Nextbot Names", Default=featureStates.NextbotESP.names }) nNames:OnChanged(function() featureStates.NextbotESP.names = nNames.Value end)
    local nDist = T:AddToggle("NextbotDistance",{ Title="Nextbot Distance", Default=featureStates.NextbotESP.distance }) nDist:OnChanged(function() featureStates.NextbotESP.distance = nDist.Value end)
    local nRainbow = T:AddToggle("NextbotRainbow",{ Title="Nextbot Rainbow Boxes", Default=featureStates.NextbotESP.rainbowBoxes }) nRainbow:OnChanged(function() featureStates.NextbotESP.rainbowBoxes = nRainbow.Value end)
    local nBoxType = T:AddDropdown("NextbotBoxType",{ Title="Nextbot Box Type", Values={"2D","3D"}, Default=featureStates.NextbotESP.boxType or "2D", Multi=false }) nBoxType:OnChanged(function(v) featureStates.NextbotESP.boxType = v end)
    T:AddDivider()
    T:AddButton({ Title="Start Player ESP", Callback=function() local ok,res = safeCallAny("startPlayerESP"); if not ok then Fluent:Notify({ Title="Player ESP", Content=tostring(res), Duration=3 }) end end })
    T:AddButton({ Title="Stop Player ESP", Callback=function() local ok,res = safeCallAny("stopPlayerESP"); if not ok then Fluent:Notify({ Title="Player ESP", Content=tostring(res), Duration=3 }) end end })
end

-- Settings Tab
do local T = Tabs.Settings
    local themeToggle = T:AddToggle("ThemeToggle",{ Title="Light Theme", Default=(Fluent:GetCurrentTheme()=="Light") }) themeToggle:OnChanged(function() if themeToggle.Value then Fluent:SetTheme("Light") else Fluent:SetTheme("Dark") end end)
    local transSlider = T:AddSlider("Transparency",{ Title="Window Transparency", Default=(Fluent.TransparencyValue or 0.2), Min=0,Max=1,Rounding=2 }) transSlider:OnChanged(function(v) Fluent.TransparencyValue = v end)
    if successSaveMgr and SaveManager then SaveManager:SetLibrary(Fluent); SaveManager:SetFolder("ZenHubConfigs"); T:AddButton({ Title="Save Config", Callback=function() pcall(function() SaveManager:Save() end) end }); T:AddButton({ Title="Load Config", Callback=function() pcall(function() SaveManager:Load() end) end }); local autoLoadToggle = T:AddToggle("AutoLoadConfig",{ Title="Autoload Saved Config", Default=false }) autoLoadToggle:OnChanged(function(v) if v then pcall(function() SaveManager:LoadAutoloadConfig() end) end end) else T:AddParagraph({ Title="Config", Content="SaveManager not available; config saving disabled." }) end
end

Window:SelectTab(1)
Fluent:Notify({ Title = "Zen Hub", Content = "Fluent UI loaded — UI replacement active.", Duration = 4 })
