...
-- Expose a Tabs table similar to other UI libraries used in the original script.
local Tabs = {
    Player = Window:AddTab({ Title = "Player" }),
    Auto = Window:AddTab({ Title = "Auto" }),
...
-- Simple compatibility 'WindUI' table so existing calls like WindUI:Notify(...) still work.
local WindUI = {}
function WindUI:Localization(...) return {} end
function WindUI:SetTheme(theme) pcall(function() Fluent:SetTheme(theme) end) end
function WindUI:GetThemes() return { Dark = true, Light = true } end
...
-- Create a persistent toggle button (mobile + desktop) to show/hide the Fluent Window
local Players = game:GetService("Players")
local Player = Players.LocalPlayer
local PlayerGui = Player:WaitForChild("PlayerGui")

local ToggleScreenGui = Instance.new("ScreenGui")
ToggleScreenGui.Name = "ZenToggleUI"
...
-- Drag support for the toggle button on mobile/PC
...
pcall(function() WindUI:Notify({ Title = "Zen Hub Loaded", Content = "Welcome, " .. Player.Name, Duration = 5 }) end)

-- Expose globals expected by the rest of DaraHub code
